...
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'sa') {
    msg.reply('Aleyküm Selam,Seni Koruyom He :D').then(s=> s.delete (1000));
  }
});
...
client.on("channelDelete", async channel => {
  ...
  if(deleter.id == "533292083172081695") return; //bu satıra kendi id'nizi yazın sizin kanal silmenizi engellemeyecektir
              ...
              if (küfür.some(word => msg.content.toLowerCase().includes(word))) {
                try {
                  if (!msg.member.hasPermission("MANAGE_WEBHOOKS")) {
                    ...
                    .setColor(0xffa300)
                    .setFooter('LinLord`S Küfür Sistemi', client.user.avatarURL)
                    .setAuthor(msg.guild.owner.user.username, msg.guild.owner.user.avatarURL)
                    .setDescription("LinLord`S, " + `***${msg.guild.name}***` + " adlı sunucunuzda küfür yakaladım.")
